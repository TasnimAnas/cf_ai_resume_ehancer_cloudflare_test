...
async function parseResume(text: string, env: Env): Promise<ParsedResume> {
  try {
...
async function parseJobLink(url: string, env: Env): Promise<ParsedJob> {
  try {
...
async function generateDocuments(
  ...
  try {
    ...
    const companyMatch = request.jobDescription.match(
      /(?:company|organization|at)\s+([A-Z][a-zA-Z\s&]+?)(?:\s+is|\s+seeks|\s+looking|\.|,)/i
    );
    ...
    const [keywords, suggestions] = await Promise.all([
      extractKeywords(request.jobDescription, env),
      generateSuggestions(request.jobDescription, request.userExperience, env)
    ]);
  ...
  async fetch(request: Request, _env: Env, _ctx: ExecutionContext) {
    ...
    if (url.pathname === "/generate" && request.method === "POST") {
      try {
          ...
          return Response.json(
            {
              success: false,
              error:
                "Missing required fields: jobDescription and userExperience"
            },
        ...
        const result = await generateDocuments(body, _env);
    ...
    if (url.pathname === "/parse-resume" && request.method === "POST") {
      try {
        ...
        const parsed = await parseResume(text, _env);
    ...
    if (url.pathname === "/parse-job-link" && request.method === "POST") {
      try {
        ...
        const parsed = await parseJobLink(body.url, _env);
